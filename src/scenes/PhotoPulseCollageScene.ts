// src/scenes/PhotoPulseCollageScene.ts
 ...
 * ----------------------
 * 写真素材を全画面で切り替えつつ、スライスや角度の変化でリズムを付与するフォトコラージュシーン。
 */
export class PhotoPulseCollageScene implements IScene {
    public name: string = 'Photo Pulse Collage';
    ...
    private overlayEase(mode: OverlayAlphaMode, beatPhase: number): number {
        switch (mode) {
            ...
            case 'pulse':
                return Easing.easeInOutCubic((beatPhase * 2) % 1);
            case 'strobe':
                return beatPhase < 0.1 ? 1 : 0.2;
            case 'steady':
            default:
                return 0.8;
        }
    ...
    private drawFallback(tex: p5.Graphics): void {
        ...
        tex.textSize(Math.min(tex.width, tex.height) * 0.06);
        tex.text('No images loaded', tex.width / 2, tex.height / 2);
        tex.pop();
    }
}
